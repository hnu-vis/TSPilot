...
class VisualizationTool(BaseTool):
    ...
    async def _audit_plan(
        ...
        prompt = prompt_locale_instruction(request_state.response_language) + (
            ...
            "Begin from the user's analytical task and the conclusions that must be visually checked, then audit whether the visualization as a whole fulfills that goal. "
            "Do not let artifact storage shapes redefine the task, and do not require one chart layer per claim or mirror the analysis artifact structure. "
    ...
    async def _plan(
        ...
        prompt = prompt_locale_instruction(request_state.response_language) + (
            ...
            "First identify the user's analytical task, the conclusion to verify, and the minimum visual roles needed to keep that verification on target. "
            ...
            "One structured Insight may support different visual projections in different layers; select exact fields for located marks and use its complete metric value for text/table presentation without requesting duplicate analysis. "
